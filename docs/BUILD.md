...
There is **no ready-to-install locked-mode app yet**. `swift run WeekPactApp` runs the usage tracker and old browser trial only. A signed filter requires an enrolled Apple Developer Program team; a free Personal Team cannot provision the Network Extensions capability.
